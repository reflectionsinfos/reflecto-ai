...
class AILearningService {
     ...
     */
    static async generateLesson(request) {
...
Return ONLY valid JSON with this exact structure (no markdown, no code blocks):
{
  ...
  "quizQuestions": [
     ...
     */
    static async evaluateQuiz(quizQuestions, userAnswers, exerciseSubmission) {
        let correctCount = 0;
...
Provide encouraging, constructive feedback in 2-3 sentences. Be specific about what they did well and what to focus on next. Keep it motivational and actionable.`;
        try {
            ...
            if (exerciseSubmission)
                pointsEarned += 5; // Bonus for completing exercise
     ...
     */
    static async suggestNextTopic(techStack, completedTopics, userGoals) {
        const model = genAI.getGenerativeModel({
            model: "gemini-2.0-flash",
            generationConfig: {
                temperature: 0.7,
