...
All three keys must be present to enable release note generation.
If any are missing the step is skipped with a WARN and deployment continues.

| Key | Default | Notes |
|-----|---------|-------|
| `RELEASE_AGENT_GROQ_API_KEY` | — | Groq API key; absent = skip |
| `RELEASE_AGENT_DIR` | `C:\projects\nexus-ai\ai-agents\release` | Path to release agent root |
...
Automatically runs after a successful `auto` deployment when all three
release-notes keys are present in `env.prod`.

**What is generated** — four files saved locally under
`deployment/release_notes/<release-name>/` and also SCP'd to the remote release:
...
**One-time venv setup for the release agent:**
